...
class RWKV(pl.LightningModule):
    ...
    def configure_model(self):
        ...
        #if self.args.actual_model_path and self.local_rank == 0:
        #    print(self.local_rank, "loading model")
        #    load_dict = torch.load(self.args.actual_model_path)
    ...
    def load_weights(self):
        ...
        if self.global_rank == 0:
            ...
            print("Loading ", self.args.actual_model_path, "on rank", self.global_rank)
            load_dict = torch.load(self.args.actual_model_path, map_location='cpu')
        ...
        for name in sorted_keys:        
            if self.global_rank == 0:
                ...
                tensor.view(-1)[0] = 1.435345354334534 # have to set this for some really strange reason but pytorch broadcast is so much faster
            ...
            if tensor.shape != size_dict[name] or torch.sum(tensor) == 1.435345354334534:
                print(tensor)
                print("The broadcast failed")
    ...
    def training_step(self, batch, batch_idx):
        ...
        #    if not hasattr(self,"loadedweights"):
        #        self.load_weights()
        ...
        #print(torch.sum(idx.view(-1, idx.size(-1))))
        
        avg_loss = loss.clone()
        
        self.trainer.strategy.reduce(avg_loss, reduce_op="mean")
        
        if self.trainer.is_global_zero:
            self.trainer.avg_loss = avg_loss

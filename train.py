...
# probably worth checking if the deepspeed function to load model params works multi node without me executing it on every node.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init_model", default=0, type=int)
    ...
    parser.add_argument("--lr_step_period", default=-1, type=int)
    parser.add_argument("--step_begin", default=0, type=int)
    parser.add_argument("--epoch_count", default=1, type=int)
    parser.add_argument("--epoch_step_save", default=1500, type=int)  # save the model every x steps
...
"""
    ...
    if args.init_model == 1:  # shall we build the initial weights?
        model = RWKV(args)
    ...
    if os.path.isfile(args.load_model):
        args.actual_model_path = args.load_model

     
    args.start_step = args.step_begin
    ...
    for p in os.listdir(args.proj_dir):
        ...
        if len(list_p) > 0:
            max_p = list_p[-1]
            if max_p == -1:
                args.actual_model_path = f"{args.proj_dir}/rwkv-init.pth"
                args.start_step = 0
            else:
    ...
    if (args.num_nodes == 1 or os.environ.get("NODE_RANK") == "0") and args.wandb != "":
    ...
    # can add more loggers or move blinks thingy into a logger
    # blinks logger logs at the end of each epoch, need to fix this
    trainer = Trainer(
    ...
    #    model.configure_model()
    #model.load_state_dict(torch.load(args.actual_model_path, map_location='cpu', weights_only=True, mmap=True))
        
    #if trainer.world_size > 1:
    #    torch.distributed.barrier()
    #    if trainer.local_rank == 0:
    #        # Broadcast the model state dictionary from local rank 0 to all other processes
    #        torch.distributed.broadcast_object_list([model.state_dict()], src=0)
    #    else:
    #        # Receive the broadcasted model state dictionary on non-zero local rank processes
    #        model_state_dict = torch.distributed.get_object_list()[0]
    ...
    # must set shuffle=False, persistent_workers=False (because worker is in another thread) / set prefetch factor
